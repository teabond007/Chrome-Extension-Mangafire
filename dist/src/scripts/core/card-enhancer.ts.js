import { STATUS_COLORS } from "/src/scripts/core/config.js.js";
...
export class CardEnhancer {
  ...
  findMatch(card, library) {
    const exactMatch = library.find(
      (e) => e.source === this.adapter.id && e.sourceId === card.data.id
    );
    if (exactMatch)
      return exactMatch;
    const normalizedTitle = this.normalizeTitle(card.data.title);
    return library.find(
      (e) => this.normalizeTitle(e.title) === normalizedTitle
    );
  }
  /**
  ...
  applyProgressBadge(card, entry) {
    ...
    const text = totalChapters ? `${unitName} ${entry.lastReadChapter}/${totalChapters}` : `${unitName} ${entry.lastReadChapter}+`;
  ...
  applyQuickActions(card, entry) {
    ...
    const callbacks = {
      continue: (entry2, btn, e) => this.handleContinueReading(entry2, card),
      status: (entry2, btn, e) => this.handleStatusChange(entry2, btn, card),
      rating: (entry2, btn, e) => this.handleRatingChange(entry2, btn, card),
      details: (entry2, btn, e) => this.handleViewDetails(entry2, card)
    };
  ...
  handleStatusChange(entry, btn, card) {
    ...
    const picker = OverlayFactory.createStatusPicker(
      entry,
      (newStatus, entry2) => this.saveStatusChange(entry2, newStatus),
  ...
  handleRatingChange(entry, btn, card) {
    ...
    const picker = OverlayFactory.createRatingPicker(
      entry,
      (rating, entry2) => this.saveRatingChange(entry2, rating)
    );
    const rect = btn.getBoundingClientRect();
    picker.style.left = `${rect.left}px`;
    picker.style.top = `${rect.bottom + 8}px`;
  ...
  async saveStatusChange(entry, newStatus) {
    try {
      ...
      const idx = entries.findIndex(
        (e) => this.normalizeTitle(e.title) === this.normalizeTitle(entry.title)
      );
      if (idx !== -1) {
        entries[idx].status = newStatus;
      }
      const bidx = bookmarks.findIndex(
        (b) => this.normalizeTitle(b.title) === this.normalizeTitle(entry.title)
      );
      if (bidx !== -1) {
