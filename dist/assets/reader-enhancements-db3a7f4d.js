var x=Object.defineProperty;var v=(p,t,e)=>t in p?x(p,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):p[t]=e;var m=(p,t,e)=>(v(p,typeof t!="symbol"?t+"":t,e),e);const y={Reading:"#4ade80",Completed:"#60a5fa","Plan to Read":"#fbbf24","On-Hold":"#f97316","On Hold":"#f97316",Dropped:"#ef4444","Re-reading":"#a855f7",HasHistory:"#9ca3af"};class c{static create(t,e,r={}){var h;const a=document.createElement("div");a.className="bmh-quick-tooltip",a.dataset.entryId=t.slug||t.id||"";const s=e.unitName==="episode"?"Ep.":"Ch.",o=parseFloat(t.lastReadChapter)||0,n=c.calculateNextChapter(t),i=c.getStatusColor(t.status),l=((h=t.personalData)==null?void 0:h.rating)||0,d=o>0;return a.innerHTML=`
            <button class="bmh-tt-btn bmh-tt-continue ${d?"":"bmh-btn-disabled"}" 
                    data-action="continue" 
                    title="${d?`Continue ${s} ${n}`:`Start Reading ${s} 1`}">
                ▶
            </button>
            ...
            <button class="bmh-tt-btn bmh-tt-rating" data-action="rating" title="Rating: ${l}/10">
                ${l>0?l:"★"}
            </button>
            <button class="bmh-tt-btn bmh-tt-info" data-action="details" title="View Details">
                ℹ
            </button>
        `,c.attachEventListeners(a,t,r),a}static createExpanded(t,e,r={}){var i;const a=document.createElement("div");a.className="bmh-quick-actions bmh-qa-expanded",a.dataset.entryId=t.slug||t.id||"";const s=e.unitName==="episode"?"Ep.":"Ch.",o=c.calculateNextChapter(t),n=((i=t.personalData)==null?void 0:i.rating)||0;return a.innerHTML=`
            <div class="bmh-qa-content">
                ...
                <button class="bmh-qa-btn bmh-qa-status" data-action="status" title="Change reading status">
                    <span class="bmh-qa-status-indicator" style="background: ${c.getStatusColor(t.status)}"></span>
                    ${t.status||"Add to Library"}
                </button>
                <div class="bmh-qa-row">
                    <div class="bmh-qa-rating" data-action="rating" title="Rate (${n}/10)">
                        ${c.renderRatingBadge(n)}
                    </div>
        ...
        `,a.querySelectorAll("[data-status]").forEach(n=>{n.addEventListener("click",i=>{i.stopPropagation(),e==null||e(n.dataset.status,t),a.remove()})}),c.autoCloseOnOutsideClick(a),a}static createRatingPicker(t,e){var o;const r=document.createElement("div");r.className="bmh-rating-picker";const a=((o=t.personalData)==null?void 0:o.rating)||0;r.innerHTML=`
        ...
        `;const s=r.querySelectorAll(".bmh-rating-num");return s.forEach((n,i)=>{n.addEventListener("mouseenter",()=>{s.forEach((l,d)=>{l.classList.toggle("hovered",d<=i)})})}),r.querySelector(".bmh-rating-grid").addEventListener("mouseleave",()=>{s.forEach(n=>n.classList.remove("hovered"))}),r.querySelectorAll("[data-rating]").forEach(n=>{n.addEventListener("click",i=>{i.stopPropagation();const l=parseInt(n.dataset.rating);e==null||e(l,t),r.remove()})}),c.autoCloseOnOutsideClick(r),r}static autoCloseOnOutsideClick(t){setTimeout(()=>{const e=r=>{t.contains(r.target)||(t.remove(),document.removeEventListener("click",e))};document.addEventListener("click",e)},0)}static autoCloseOnOutsideClick(t){setTimeout(()=>{const e=r=>{t.contains(r.target)||(t.remove(),document.removeEventListener("click",e))};document.addEventListener("click",e)},0)}static injectStyles(){if(document.getElementById("bmh-overlay-styles"))return;const t=document.createElement("style");t.id="bmh-overlay-styles",t.textContent=`
            /* ===== COMPACT TOOLTIP ===== */
            .bmh-quick-tooltip {
                position: absolute;
                top: 8px;
